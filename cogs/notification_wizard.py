...
class WizardSession:
    ...
    def _reconstruct_event_data(self, event_type: str, notifications: list):
        ...
        elif event_type == "Tri-Alliance Clash":
            for notif in notifications:
                instance = notif.get("instance_identifier", "legion1")
                if instance == "legion1":
        ...
        elif event_type == "Eternity's Reach":
...
class EventConfigRouter:
    ...
    async def route(self, interaction: discord.Interaction):
        ...
        view_mapping = {
            ...
            "Viking Vengeance": VikingVengeanceConfigView(self.cog, self.session, self.hub_view),
            "Swordland Showdown": SwordlandShowdownConfigView(self.cog, self.session, self.hub_view),
            "Tri-Alliance Clash": TriAllianceClashConfigView(self.cog, self.session, self.hub_view),
...
class VikingVengeanceConfigView:
    ...
    async def show(self, interaction: discord.Interaction):
        """Show Viking Vengeance configuration - directly show modal"""
        modal = VikingVengeanceModal(self.cog, self.session, self.hub_view)
        await interaction.response.send_modal(modal)
...
class VikingVengeanceModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        """Process Viking Vengeance (Crazy Joe) configuration"""
        try:
            ...
            # Mark as configured and return to hub
            self.session.mark_event_configured("Viking Vengeance")
            await self.hub_view.show(interaction)
...
class CaesaresFuryConfigView(discord.ui.View):
    ...
    async def show(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                f"Caesares Fury occur **every 3 weeks during a 3-day window**.\n\n"
...
class CaesaresFuryBossTimeModal(discord.ui.Modal, title="Add Caesares Fury Boss"):
    def __init__(self, config_view: 'CaesaresFuryConfigView'):
        super().__init__()
...
class CaesaresFuryAllBossesModal(discord.ui.Modal, title="All Bosses At Once"):
    def __init__(self, config_view: 'CaesaresFuryConfigView'):
        super().__init__()
        self.config_view = config_view
...
class WizardPreviewView(discord.ui.View):
    ...
    async def show(self, interaction: discord.Interaction):
        ...
        for event in self.session.selected_events:
            ...
            if event == "Bear Trap" and data:
            ...
            elif event == "Viking Vengeance" and data:
                ...
                if tue_hour is not None and tue_min is not None:
                    ...
                    embed.add_field(
                        name=f"{icon} Viking Vengeance",
                        value=value,
                        inline=False
                    )
            elif event in ["Swordland Showdown", "Tri-Alliance Clash"] and data:
                times_str = ""
            ...
            elif event in ["Fortress Battle", "Eternity's Reach", "Castle Battle", "KvK"] and data:
            ...
            for event_name in self.session.selected_events:
                ...
                if event_name == "Bear Trap":
                    ...
                    if tuesday_hour is not None and next_tuesday:
                        ...
                        updated_count += u
                        if action != "updated":
                            display = self._get_instance_display_name("Viking Vengeance", "tuesday")
                            event_changes.setdefault(event_name, []).append((display, action))
                    ...
                    if thursday_hour is not None and next_thursday:
                        ...
                        updated_count += u
                        if action != "updated":
                            display = self._get_instance_display_name("Viking Vengeance", "thursday")
                            event_changes.setdefault(event_name, []).append((display, action))

                elif event_name == "Swordland Showdown":
                ...
                elif event_name == "Tri-Alliance Clash":
                ...
                elif event_name in ["Eternity's Reach", "Castle Battle", "KvK"]:
