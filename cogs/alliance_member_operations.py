...
_LEVEL_HEADERS = {"tc_level", "tc level", "fc_level", "fc level", "furnace", "furnace_lv", "level"}
...
class MemberListView(discord.ui.View):
    ...
    def _reload_members(self):
        """Re-read names/levels/kingdom/power so the list shows what is saved."""
        for m in self.all_members:
    ...
    async def _on_import(self, interaction: discord.Interaction):
        ...
        instructions = discord.Embed(
            ...
            description=(
                ...
                f"• Add **`Name`**, **`TC Level`**, **`Kingdom`**, **`Power`** or **`Combat Power`** "
                f"columns to set them. Members already in the alliance get updated, new ones "
                f"are added already named.\n"
                f"• Levels take either form: `82` or `TC 10-2`. Power takes `134512345` or `134.5M`.\n"
        ...
        try:
            ...
            while index < len(fids_to_process):
                ...
                try:
                    ...
                    if not kid_ok:
                        state_error = KINGDOM_CHECK_UNAVAILABLE
                    elif locked_kid is not None and not verified:
                        state_error = kingdom_lock_reason(locked_kid, kid)
                    else:
                        state_error = None
                    ...
                    # Kingdom comes from the add-path check above so locks still apply;
