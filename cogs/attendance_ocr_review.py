"""Review UI for unified Swordland Showdown/Tri-Alliance Clash sessions: registration, result, or both.
...
class EventReviewView(discord.ui.View):
    """Review for a Swordland Showdown/Tri-Alliance Clash event — mode auto-detected from the row lists."""
    ...
    def zero_is_absent(self) -> bool:
        """0-point result = absent (Swordland/Tri-Alliance); not for power/simple."""
    ...
    def _build_merged_view(self) -> list[dict]:
        ...
        for row in rows:
            ...
            if zero_absent and rv == 0:
                ...
                # Game rule: a FID can only join one Swordland Showdown/Tri-Alliance Clash per cycle,
    ...
    def _build_scoreboard_embed(self, session_id: str, absent_rows: list[dict]) -> discord.Embed:
        ...
        if total_all > 0:
            ...
            if prev_total and prev_total > 0:
                ...
                arrow = f"{theme.averageIcon}" if delta_pct >= 0 else "📉"
                analytics_lines.append(
...
async def _add_unknown_fid(interaction, view, fid, new_name=None):
    """Add a kingdom-checked unknown fid under `new_name` -> (added, nickname, note); never moves other alliances' players."""
    ...
    if alliance_kid is None:
        return False, None, (f"{theme.warnIcon} This alliance has no kingdom set, so I can't confirm ID `{fid}`. "
    ...
    if not verified:
        return False, None, (f"{theme.deniedIcon} Couldn't confirm ID `{fid}` in kingdom `{alliance_kid}`. "
                             f"They may be in another kingdom, or the ID is wrong. Add them under Alliance Management first.")
