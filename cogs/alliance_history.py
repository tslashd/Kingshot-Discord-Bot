"""
Alliance history cog. Tracks and displays nickname and town center level changes.
"""
import discord
...
class AllianceHistory(commands.Cog):
        ...
        try:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"{theme.stoveIcon} **Castle Changes**\n"
                    f"└ View town center level changes\n\n"
        ...
        try:
            ...
            if not changes:
                await interaction.followup.send(
                    "No town center changes found for this player.",
                    ephemeral=True
                )
                return
            ...
            await interaction.followup.send(
                f"{theme.deniedIcon} An error occurred while displaying the town center history.",
                ephemeral=True
            )
    ...
    async def show_member_list_furnace(self, interaction: discord.Interaction, alliance_id: int):
        """Show the member list for town center history selection (skip alliance picker)."""
        try:
            with sqlite3.connect('db/users.sqlite') as users_db:
                cursor = users_db.cursor()
                cursor.execute(
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"Select a member to view town center history:\n"
                    f"{theme.upperDivider}\n"
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"{theme.upperDivider}\n"
                    f"{theme.stoveIcon} **Castle Changes** — track TG level changes over time\n"
...
class HistoryTypeView(discord.ui.View):
    ...
    @discord.ui.button(label="Castle Changes", emoji=f"{theme.stoveIcon}",
                       style=discord.ButtonStyle.primary, row=0)
    ...
    async def furnace_changes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            select_embed = discord.Embed(
                title=f"{theme.stoveIcon} Castle Changes",
                description=(
                    f"Select an alliance to view town center changes:\n\n"
    ...
    def update_view(self):
        ...
        async def member_callback(interaction):
            try:
                ...
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        f"{theme.deniedIcon} An error occurred while showing town center history.",
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
