...
class BotHealth(commands.Cog):
    def __init__(self, bot):
        ...
        self.gift_api_url = "http://ks-gift-code-api.whiteout-bot.com/giftcode_api.php"
    ...
    async def check_wos_api_status(self) -> dict:
        """Cached Kingshot API status. Returns cache when fresh; otherwise probes
    ...
    async def _probe_wos_api(self) -> dict:
        ...
        try:
            ...
            if status['api1_available']:
                return {'status': STATUS_HEALTHY, 'message': 'Online'}
            else:
                return {'status': STATUS_ERROR, 'message': 'Unavailable'}

        except Exception as e:
            self.logger.error(f"Error checking Kingshot API status: {e}")
