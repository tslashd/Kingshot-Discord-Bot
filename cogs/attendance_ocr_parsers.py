...
# Kingshot reskin note: `key` values stay (internal identifiers shared with the
# DB / EVENT_TYPES); `label`/`default_keywords` use the Kingshot event names
# (Foundry Battle → Swordland Showdown, Canyon Clash → Tri-Alliance Clash,
# Alliance Showdown → Alliance Brawl).
# Fingerprint status against real Kingshot screenshots:
#   - Swordland Showdown RESULT mail: VERIFIED (2026-05-31). Values are
#     comma-formatted Personal Relic Points, so row parsing works.
#   - Swordland Showdown REGISTRATION mail: detection VERIFIED (2026-05-29). The
#     per-combatant value is Combat Power, which is small/unbloated (KS: 432, and
#     2-digit on fresh servers) — handled via _FORMATTED_NUMBER_RE_SMALL (2-digit
#     floor), gated to foundry_battle registration only. Writes to
#     users.combat_power, matching WOS.
#   - Tri-Alliance Clash (both kinds), Power Rankings, Alliance Brawl result
#     anchors: still Whiteout wording — need Kingshot screenshots.
EVENT_TYPES: dict[str, EventTypeConfig] = {
    "foundry_battle": EventTypeConfig(
        key="foundry_battle",
        label="Swordland Showdown",
        default_keywords=("Swordland Showdown", "Swordland"),
        fingerprint_re_by_kind={
            # KS Swordland Showdown registration mail (verified from screenshot 2026-05-29):
            # "selected as a Swordland Showdown [Legion N] combatant" + "Combatants: N/N".
            "registration": re.compile(
                r"(?:selected\s+as\s+a\s+Swordland\s+Showdown\b"
                r"|Swordland\s+Showdown.*?Combatants:\s*\d+\s*/\s*\d+"
                r"|list\s+of\s+combatants\s+and\s+substitutes)",
                re.IGNORECASE | re.DOTALL,
            ),
            # KS Swordland Showdown result mail (verified from screenshot 2026-05-31):
            # "Swordshrine control duration", "[Relic Points] from ...", "Personal Relic Points".
            "result": re.compile(
                r"(?:Swordshrine\s+control|Personal\s+Relic\s+Points|Relic\s+Points\]\s+from)",
                re.IGNORECASE,
            ),
        },
        event_weekday=6,
    ...
    "canyon_clash": EventTypeConfig(
        ...
        default_keywords=("Tri-Alliance Clash", "Tri-Alliance"),
        fingerprint_re_by_kind={
            "registration": re.compile(
                _REGISTRATION_MARKERS + r".*?Tri-Alliance",
                re.IGNORECASE | re.DOTALL,
            ),
            # KS Tri-Alliance Clash result mail (verified 2026-06-13): scoreboard
            # header "ranked No. N in [Tri-Alliance Clash]" + "Total Energy Used"
            # stat, and the "Personal Point Ranking" leaderboard page.
            "result": re.compile(
                r"(?:Personal\s+Point\s+Ranking"
                r"|ranked\s+No\.?\s*\d+\s+in\s+\[?Tri-Alliance"
    ...
    "alliance_showdown": EventTypeConfig(
        ...
        default_keywords=("Alliance Brawl",),
        fingerprint_re_by_kind={
            "result": re.compile(r"Alliance\s+Brawl", re.IGNORECASE),
        },
    ),
}
...
def resolve_event_date(mail_date_local: date, event_type: str,
    ...
    if registration:
        # The registration mail arrives before the event (Swordland ~2 days,
        # Tri-Alliance ~1) — snap forward to the event's weekday so it dates the
        # event, not the sign-up day, and matches the result session.
...
_RESULT_STAT_KEYS: tuple[tuple[str, str], ...] = (
    ("fuel_used",        r"Total\s+Energy\s+Used"),
...
# Combat Power (Foundry / Swordland Showdown registration) is small and unbloated
# — on newer servers it can be a few hundred or even 2 digits (a fresh farm
# account). The default matcher's 4-digit floor misses those, so this variant
# drops the floor to 2 digits. Safe here because alliance ranks are single-digit
# (R1-R5), so 2-digit values never collide with a rank. Use it ONLY for that
# registration kind, not for result/Power pages where small bare numbers would be
# noise (dates, reward counts).
_FORMATTED_NUMBER_RE_SMALL = re.compile(
    r"\d+(?:\.\d+)?[KkMmBb]\b"
    r"|\d{1,3}(?:[,\.]\d{3})+"
...
_PARSE_STOPWORDS = frozenset({
    ...
    "chief", "power", "ko", "contribution", "claimed", "no", "governor",
})
...
_SECTION_MARKERS = (
    "Personal Relic Points",     # KS Swordland Showdown result leaderboard header
...
_END_MARKERS = (
    "Expires in",
    "Rewards",          # KS result mails: reward icons follow the leaderboard
    "Battle Overview",
    "Master Bonus",
    "Expert Bonus",
)
...
_RESULT_HEADER_MARKERS = (
    ...
    # KS Swordland Showdown result header
    "swordshrine control", "sieged by mercenaries", "relic points] from", "battle stats",
)
_RESULT_LEADERBOARD_MARKERS = (
    "personal arsenal points", "personal point ranking",
    "personal relic points",   # KS Swordland Showdown
)
...
def find_formatted_numbers(text: str, *, allow_small: bool = False) -> list[tuple[int, int, int]]:
    out = []
    regex = _FORMATTED_NUMBER_RE_SMALL if allow_small else _FORMATTED_NUMBER_RE
...
def _parse_player_value_rows(text: str, *, allow_small: bool = False,
                             capture_tail: bool = False) -> list[dict]:
    ...
    section only). `allow_small` drops the value floor to 2 digits for small
    Combat Power registration values (Foundry / Swordland Showdown).
    `capture_tail` (results only) recovers trailing 0-scorers."""
    rows = []
    text = _trim_to_data_section(text)
    prev_end = 0
    for start, end, value in find_formatted_numbers(text, allow_small=allow_small):
...
class OcrUploadSession:
    ...
    async def _phase_callback(self, phase: str, lang: str):
        """Update the live OCR phase/language so the progress embed shows which language pass is running."""
...
# ── Swordland Showdown / Tri-Alliance Clash unified session: registration + result in one ────────

class _PointsSession(OcrUploadSession):
    """Unified Swordland Showdown/Tri-Alliance Clash session. Collects registration + result screenshots
    ...
    async def _process_attachments(self, attachments: list[discord.Attachment]):
        ...
        for att in attachments:
            ...
            # Foundry/Swordland registration captures small Combat Power values;
            # other pages use the 4-digit floor to avoid small-number noise.
            allow_small = kind == "registration" and self.db_event_type == "foundry_battle"
            ...
            if not header_page:
                ...
                _tail = kind == "result"
                parse = lambda t, _s=allow_small, _c=_tail: _parse_player_value_rows(
                    t, allow_small=_s, capture_tail=_c)
                try:
    ...
    def _merge_result_metadata(self, text: str, blocks: Optional[list] = None):
        ...
        if self.alliance_rank is None:
            ...
            if m:
                ...
                # Swordland Showdown has no "ranked No. N" — derive 1/2 from win/loss wording.
...
class FoundryBattleSession(_PointsSession):
    ...
    result_value_label = "Personal Relic Points"
...
class AllianceShowdownSession(OcrUploadSession):
    """Alliance Brawl final-rankings parser: per-player points, routed through
    the shared review (result-only) for the same fix-at-upload + edit-later
    lifecycle as Swordland Showdown/Tri-Alliance Clash."""

    db_event_type = "alliance_showdown"
    registration_value_label = "Power"        # unused — Brawl has no sign-up phase
    result_value_label = "Brawl Points"
    simple_results = True                      # no registration / present-absent concept
