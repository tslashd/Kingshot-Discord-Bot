"""Editing member names, Town Center levels, kingdoms and power by hand.
...
def apply_member_edit(fid, *, nickname=None, furnace_lv=None, kid=None,
    ...
    """Write a member's name/level/kingdom/power, limited to `alliance_id` when given; returns what changed."""
...
_EDIT_LINE_COLUMNS = (
    (parse_furnace_level, "isn't a Town Center level (try `80` or `TC 10`)"),
    (parse_state, "isn't a kingdom number (try `911`)"),
...
def parse_edit_line(line):
    """`id, name, level, kingdom, power, combat power` (all but the id optional) -> 6-tuple, or an error string."""
...
class MemberEditModal(discord.ui.Modal):
    """Edit one member's name, Town Center level, kingdom and power."""
    ...
    def __init__(self, parent_view, fid, nickname, furnace_lv, alliance_id, kid=None,
        ...
        self.level_input = _optional_input(
            "Town Center level (optional)", "TC 10-2, or 82",
            format_furnace_level(furnace_lv) if furnace_lv else "", 20)
        self.state_input = _optional_input("Kingdom (optional)", "911", str(kid) if kid else "", 5)
    ...
    def _parsed_fields(self):
        ...
        return (
            ("furnace_lv", self.level_input, parse_furnace_level,
             "isn't a Town Center level. Try a number like `82`, or the in-game name like `TC 10-2`."),
            ("kid", self.state_input, parse_state,
             "isn't a kingdom number. Enter digits only, like `911`."),
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if "state" in changed and enqueue_catchups(self.parent_view.cog.bot, [self.fid]):
            note = f"Redeeming the codes `{self.fid}` missed while their kingdom was wrong."
        await self.parent_view.refresh_after_edit(interaction, note)


class BulkMemberEditModal(discord.ui.Modal):
    """Edit a whole list of members in one go, one `id, name, level, kingdom, power, CP` per line."""
    ...
    def __init__(self, parent_view, alliance_id, prefill=""):
        ...
        self.lines_input = discord.ui.TextInput(
            label="Per line: id, name, level, kingdom, power, CP",
            style=discord.TextStyle.paragraph,
            placeholder="306234280, THANOS IS RIGHT, TC 10-2, 259, 134.5M, 20.1M\n123051289, SKORN, 80, 259",
            default=prefill,
            required=True,
            max_length=4000,
        )
