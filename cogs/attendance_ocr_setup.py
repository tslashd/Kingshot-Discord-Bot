...
# the event_type used in EVENT_TYPES. Swordland Showdown / Tri-Alliance Clash
# entries cover BOTH the pre-event registration mail AND the post-event result
# mail — the bot auto-detects which kind a screenshot is and merges them into
# one session.
# TODO(KS screenshots): verify the exact Kingshot in-game mail submenu wording
# (the "Solo Participation"/"Legion X Battle Result" labels are Whiteout's).
_EVENT_LOCATIONS: dict[str, str] = {
    "foundry_battle":    "Mail → Alliance → [Swordland Showdown] Registration Ends *and* Results",
    "canyon_clash":      "Mail → Alliance → [Tri-Alliance Clash] Solo Participation *and* Legion X Battle Result",
    "power_rankings":    "Alliance → Power Rankings",
    "alliance_showdown": "Mail → Alliance → Alliance Brawl Ranking",
}
...
def render_info_message(channel_id: int) -> str:
    ...
    lines.append(
        ...
        "For Swordland Showdown/Tri-Alliance Clash, include BOTH the registration mail AND the result mail "
...
class _KeywordsModal(discord.ui.Modal):
    ...
    def __init__(self, parent: _KeywordsView, *, event_type: str,
        ...
        self.input = discord.ui.TextInput(
            ...
            placeholder="Swordland Showdown\nSwordland",
            required=False,
            max_length=500,
        )
