...
async def ocr_rows_with_fallback(image_bytes: bytes, *, primary_lang: str,
    ...
    """
    # Reuse a primary read the caller already has (Tri-Alliance Clash / Swordland
    # does a boxed pass for the scoreboard) so the primary engine isn't run twice.
...
class EditRowModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction):
        ...
        if err:
            ...
            return
        # Shared resolver: roster match, or kingdom-confirm + add-to-alliance for an unknown ID.
...
async def _resolve_and_apply(interaction, view, *, row_id, text, damage, rank, raw_name,
    ...
    unknown ID is confirmed against the alliance's kingdom and offered for
...
async def _offer_add_by_id(interaction, view, *, row_id, fid, damage, rank, raw_name, new_name=None):
    """Confirm an unknown fid is in the alliance's kingdom (one probe), then show the add confirm card."""
    await interaction.response.defer()
    ...
    if alliance_kid is None:
        await interaction.followup.send(
            f"{theme.warnIcon} This alliance has no kingdom set, so I can't confirm ID `{fid}`. "
            f"Set its kingdom (Alliance Management → Set Kingdom), or add the member under Manage Members first.",
            ephemeral=True)
        return
    kid, verified = await gift_state_resolver.verify_add_state(gift_cog, fid, view.alliance_id)
    if not verified:
        await interaction.followup.send(
            f"{theme.deniedIcon} Couldn't confirm ID `{fid}` in kingdom `{alliance_kid}`. "
            f"They may be in a different kingdom, or the ID is wrong. Add them under Alliance Management → "
...
class PlayerAddConfirmView(discord.ui.View):
    """Confirm adding a kingdom-checked fid to the alliance under the typed name, and matching the row."""
    ...
    def build_embed(self) -> discord.Embed:
        ...
        return discord.Embed(
            ...
            description=(
                ...
                f"**{theme.globeIcon} Kingdom:** `{self.kid}` (confirmed)\n"
...
class BearChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            # alliance for Bear or Screenshot Upload. Attendance OCR isn't ported
            # to Kingshot yet, so the check is skipped if that module is absent.
            try:
                from .attendance_ocr_setup import (
                    find_conflicting_channel_owner, format_channel_conflict,
                )
            except ImportError:
                find_conflicting_channel_owner = None
            if find_conflicting_channel_owner is not None:
...
class DataSubmit:
    ...
    async def _persist_hunt_and_render(self, interaction, *, date, hunting_trap, rallies,
        ...
        # freezes the event loop). Non-fatal here — the hunt is already committed.
