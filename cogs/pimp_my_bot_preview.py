...
class ThemePreviewView(discord.ui.View):
    ...
    def build_changes_preview(self) -> discord.Embed:
        ...
        # Town Center level change example
        embed.add_field(
            name=f"{user_icon} KingsKnight",
            value=f"{stove_old_icon} `TC 5` ➜ {stove_icon} `TC 6`\n{time_icon} Just now",
            inline=False
        )

        # Nickname change example
        embed.add_field(
            name=f"{user_icon} RoyalHighness",
            value=f"{avatar_old_icon} `OldNickname` ➜ {avatar_icon} `RoyalHighness`\n{time_icon} 5 min ago",
            inline=False
        )
    ...
    def build_member_info_preview(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"{alliance_icon} **Alliance:** Really Cool Alliance\n"
                f"{stove_icon} **Town Center:** TC 8-3\n"
    ...
    def build_player_lookup_preview(self) -> discord.Embed:
        ...
        thumbnail for town center level content. This preview helps users see how
        ...
        # Thumbnail would be the town center level image
